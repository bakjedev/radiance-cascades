...
inline vk::UniquePipelineLayout create_pipeline_layout(vk::Device device,
                                                       ...
                                                       std::span<const vk::PushConstantRange> push = {})
{
  return device.createPipelineLayoutUnique(
      vk::PipelineLayoutCreateInfo{}.setSetLayouts(sets).setPushConstantRanges(push));
}
...
inline constexpr auto blend_opaque = [] {
  ...
  blend.setColorWriteMask(vk::ColorComponentFlagBits::eR | vk::ColorComponentFlagBits::eG |
                                                   ...
                                                   const vk::ShaderModuleCreateFlags flags = {})
{
  if (code.empty() || code.size() % sizeof(uint32_t) != 0) {
    throw std::runtime_error("Shader code size must be non zero and a multiple of 4. Size: " +
                             std::to_string(code.size()));
  }

  return device.createShaderModuleUnique(
      vk::ShaderModuleCreateInfo{flags, code.size(), reinterpret_cast<const uint32_t*>(code.data())});
}
