...
Window::Window(EventDispatcher& event_dispatcher, const uint32_t width, const uint32_t height, const char* title) :
    ...
    handle_(SDL_CreateWindow(title, static_cast<int>(width), static_cast<int>(height),
                             SDL_WINDOW_RESIZABLE | SDL_WINDOW_VULKAN)),
    width_(width), height_(height), resize_listener_(event_dispatcher_->listen<WindowResizeEvent>(
...
Window::~Window() { event_dispatcher_->remove(resize_listener_); }
