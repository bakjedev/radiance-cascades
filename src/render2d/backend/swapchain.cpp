...
Swapchain::Swapchain(const Device& device, vk::Extent2D window_extent, vk::ImageUsageFlags image_usage) :
    device_(&device), image_usage_(image_usage)
{
  create(window_extent);
}

Swapchain::~Swapchain() { destroy_views(); }
